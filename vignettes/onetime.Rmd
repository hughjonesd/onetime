...
Onetime is a lightweight package, with just two
package dependencies, rappdirs and filelock. Its total size including
dependencies is less than 50 Kb. So it is cheap to include as a dependency in
your own package.
...
```{r}
id <- "vignette-3"
onetime_warning("What are you doing with that woman?", id = id)
```
...
```{r}
id <- "vignette-5"
onetime_do(print(getRversion()), id = id)
```
...
```{r}
id <- "vignette-7"
onetime_message_confirm("Hans, have you got the weedkiller?", id = id)
```
...
As a package author, you have several options.
...
* If your package is used directly from the command line, the simplest option
  ...
  user for permission to store files if it has not been granted. Note that the
  user may say no; if so the action may be run more than once!
...
ids <- paste0("vignette-", 1:8)
for (id in ids) onetime_reset(id)
```

Conversely, you can use `onetime_mark_as_done()` to manually mark a particular
action as done by `id`. 
